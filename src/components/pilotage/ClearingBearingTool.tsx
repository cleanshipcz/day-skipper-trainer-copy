...
export const ClearingBearingTool = ({ onAllScenariosComplete }: Props) => {
  ...
  return <Card className="w-full border-2 border-primary/20">
    ...
    <CardContent className="space-y-5">
      ...
      {result && <div ref={feedbackRef} id={feedbackId} tabIndex={-1} role={result.kind === "incorrect" || result.kind === "invalid" ? "alert" : "status"} aria-live={solved ? "polite" : "assertive"} aria-atomic="true" className={`flex scroll-mt-24 gap-2 rounded-lg p-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring ${solved ? "bg-green-100 text-green-900" : "bg-red-100 text-red-900"}`}>{solved ? <CheckCircle2 aria-hidden="true" className="h-5 w-5 shrink-0" /> : <XCircle aria-hidden="true" className="h-5 w-5 shrink-0" />}<span>{result.message}</span></div>}
