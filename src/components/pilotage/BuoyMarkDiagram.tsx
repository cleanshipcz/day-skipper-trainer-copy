...
const Top = ({kind}:{kind:Topmark}) => {
 const cone=(y:number,down=false)=><path d={down?`M40 ${y} L54 ${y} L47 ${y+18}Z`:`M40 ${y+18} L54 ${y+18} L47 ${y}Z`} />;
 ...
 if(kind==="cones-base") return <>{cone(3,true)}{cone(24)}</>;
 if(kind==="cones-point") return <>{cone(3)}{cone(24,true)}</>;
