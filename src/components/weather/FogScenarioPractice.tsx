  ...
  return <section aria-labelledby="fog-practice-heading" className="min-w-0 space-y-4 rounded-lg border p-4 sm:p-6 forced-colors:border-[CanvasText]">
    ...
    <article className="min-w-0 space-y-4 rounded-md bg-muted/40 p-3 sm:p-5">
      ...
      <figure className="overflow-hidden rounded-md border bg-background p-3 forced-colors:border-[CanvasText]">
        <svg viewBox="0 0 720 150" role="img" aria-labelledby={`fog-visual-${scenario.id}`} className="h-auto w-full text-foreground">
        ...
        <figcaption className="mt-2 text-sm text-muted-foreground"><span className="font-medium text-foreground">Diagram meaning: </span>{scenario.visual.caption}</figcaption>
      </figure>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:justify-between"><Button type="button" variant="outline" disabled={index === 0} onClick={() => move(index - 1)}>Previous scenario</Button><Button type="button" variant="outline" disabled={index === fogScenarios.length - 1} onClick={() => move(index + 1)}>Next scenario</Button></div>
    </article>
  </section>;
};
