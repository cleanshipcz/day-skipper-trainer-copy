...
describe("PrepareAppliedExercise", () => {
  ...
  it("requires every PREPARE objective, remediates unsafe answers, and saves one reviewable artifact", async () => {
    ...
    render(<PrepareAppliedExercise onComplete={complete}/>);
    expect(screen.getByRole("button", { name: "Save evidence-based completion" }).hasAttribute("disabled")).toBe(true);
    await user.click(screen.getByRole("button", { name: /Continue on the planned route/ }));
    expect(screen.getByRole("status").textContent).toMatch(/Not yet.*safety margins/);
  ...
  it("restores a valid revisioned draft and rejects malformed or stale data", () => {
    localStorage.setItem("prepare-applied-exercise", JSON.stringify({ catalogueRevision: PREPARE_EXERCISE_REVISION, scenarioId: "visibility-loss", responses: [prepareScenarios[1].answers[0]] }));
    const { unmount } = render(<PrepareAppliedExercise onComplete={vi.fn()}/>);
    expect(screen.getByText(/Step 2 of 7: Regulations/)).toBeTruthy(); unmount();
    localStorage.setItem("prepare-applied-exercise", "{bad");
    const malformed = render(<PrepareAppliedExercise onComplete={vi.fn()}/>);
    expect(screen.getByText(/Step 1 of 7: Passage appraisal/)).toBeTruthy(); malformed.unmount();
    localStorage.setItem("prepare-applied-exercise", JSON.stringify({ catalogueRevision: "retired", scenarioId: "visibility-loss", responses: prepareScenarios[1].answers }));
    render(<PrepareAppliedExercise onComplete={vi.fn()}/>);
  ...
  it("keeps retry evidence, supports existing completions, and exposes accessible responsive controls", async () => {
    ...
    const { rerender } = render(<PrepareAppliedExercise onComplete={complete}/>);
    for (const answer of prepareScenarios[0].answers) await user.click(screen.getByRole("button", { name: answer }));
    ...
    rerender(<PrepareAppliedExercise alreadyCompleted onComplete={complete}/>);
    expect(screen.getByRole("button", { name: "Completion saved" }).hasAttribute("disabled")).toBe(true);
