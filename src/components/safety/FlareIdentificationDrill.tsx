...
export const FlareIdentificationDrill = ({ onComplete, scenarioBank = flareScenarios, reviewApproved = isFlareContentReleased }: Props) => {
  ...
  const save = async () => {
    ...
    const evidence = { revision: FLARE_DRILL_REVISION, ownerId, mastered: true, masteredScenarioIds: ids, attempts, scope: "visual recognition and scenario choice only; not practical pyrotechnic competence", qualifiedReview: "waived; no practitioner approval claimed", completionOutcome: (ownerId ? "confirmed" : "anonymous") as "confirmed" | "anonymous" };
    try {
