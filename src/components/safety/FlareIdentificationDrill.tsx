...
import { flareScenarios, flareTypes, type FlareScenario } from "@/data/flareTypes";
...
interface Props { readonly onComplete?: (result: DrillResult) => void; readonly scenarioBank?: readonly FlareScenario[]; }
...
const validEvidence = (value: unknown, ownerId: string | null, ids: readonly string[]) => {
  ...
  return record.revision === FLARE_DRILL_REVISION && record.ownerId === ownerId && record.mastered === true &&
    Array.isArray(record.masteredScenarioIds) && ids.every(id => (record.masteredScenarioIds as unknown[]).includes(id));
};

export const FlareIdentificationDrill = ({ onComplete, scenarioBank = flareScenarios }: Props) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      let local = false;
      try { local = validEvidence(JSON.parse(localStorage.getItem(evidenceKey(ownerId)) ?? "null"), ownerId, ids); } catch { local = false; }
      const remote = await loadProgressDetailed(TOPIC_IDS.SAFETY_FLARES_DRILL);
      if (!current) return;
      const history = remote.record?.answers_history;
      const restored = remote.status === "remote" && validEvidence(history, ownerId, ids);
      if (restored || local) {
        setSaveState(restored ? "confirmed" : ownerId ? "queued" : "anonymous");
        setRoundIds([]); setAnnouncement(restored ? "Flare drill mastery restored from your account." : ownerId ? "Flare drill mastery restored from this device; remote sync is queued." : "Anonymous mastery restored on this device.");
  ...
  }, [ids, loadProgressDetailed, ownerId]);
  ...
  const advance = () => {
    ...
    if (index + 1 < roundIds.length) { setIndex(index + 1); setSelected(null); setSettled(false); setNextReady(false); setAnnouncement(`Scenario ${index + 2} of ${roundIds.length}.`); return; }
    if (missed.length) { setRoundIds(missed); setMissed([]); setIndex(0); setSelected(null); setSettled(false); setNextReady(false); setAnnouncement(`Remediation round: retry ${missed.length} missed ${missed.length === 1 ? "scenario" : "scenarios"}.`); return; }
    setRoundIds([]); setIndex(0); setSelected(null); setSettled(false); setNextReady(false); setSaveState("ready"); setAnnouncement("Mastery achieved. Save this evidence to complete the drill.");
  };
  ...
  const save = async () => {
    ...
    const evidence = { revision: FLARE_DRILL_REVISION, ownerId, mastered: true, masteredScenarioIds: ids, attempts, scope: "visual recognition and scenario choice only; not practical pyrotechnic competence", qualifiedReview: "pending" };
    try {
      ...
      localStorage.setItem(evidenceKey(ownerId), JSON.stringify(evidence));
