...
export const RigEvidencePractice = ({ onComplete }: { onComplete: () => void }) => {
  ...
  if (done) return <div ref={stepHeadingRef} tabIndex={-1} className="rounded-lg border-2 border-success p-4 outline-none focus-visible:ring-2 focus-visible:ring-ring"><strong>Evidence walk-round practice complete.</strong><p className="text-sm text-muted-foreground">You located and evaluated all three observations. Continue to the completion summary and Rig quiz link below. This practice is not evidence about a real vessel.</p></div>;
  const checkLocation = () => {
  ...
  return <div className="min-w-0 rounded-lg border p-4 forced-colors:border-[CanvasText]" data-testid="rig-evidence-practice" aria-describedby="rig-practice-instructions">
    ...
    {!locationCorrect && <Button className="mt-3 min-h-11 min-w-11" onClick={checkLocation}>Check location</Button>}
