...
import { calculateLegEtas, parseWaypointCoordinate, totalRouteDistance, type PlanLeg, type PlanWaypoint } from "@/features/passagePlanning/calculations";
import {
...
const initialPlan = (): PassagePlan => ({ version:PASSAGE_PLAN_CACHE_VERSION, name:"Solent practice passage", departure:"2026-07-30T09:00", speed:5, fuelRate:2, reservePercent:20, points:example, coordinateFormat:"degrees-decimal-minutes", datum:"WGS84", coordinatePrecision:"0.1 minute (about 185 m latitude)", safety:{departureBerth:"Portsmouth berth to harbour entrance",destinationBerth:"Bembridge harbour entrance to allocated berth",limits:"Wind no more than F5; visibility at least 3 nm; remain within vessel draft and crew limits",abortDecision:"Delay before departure or abort/divert before Bembridge approach if any limit is exceeded",alternatives:"Return Portsmouth; divert Cowes; hold offshore only if conditions and traffic permit",manualVerification:"Skipper independently plots the route and checks hazards, depths, clearance, tides and notices on current official sources before use"},provenance:{weather:"Current Met Office Shipping/Inshore Waters forecast: record issue and validity",tide:"Current official tide tables and tidal stream atlas: record edition and ports",chart:"Current official chart: record number, edition and corrections",publications:"Current almanac, sailing directions and notices to mariners",preparedAt:"2026-07-30T08:00",revisedAt:"2026-07-30T08:00"} });
...
export function PassagePlanBuilder() {
  ...
  return <div className="space-y-5">
    ...
    <fieldset className="grid gap-3 rounded-md border p-4 sm:grid-cols-2"><legend className="font-semibold">Berth-to-berth safety and PREPARE decisions</legend>{(["departureBerth","destinationBerth","limits","abortDecision","alternatives","manualVerification"] as const).map(key=><div key={key}><Label htmlFor={`safety-${key}`}>{({departureBerth:"Departure berth and exit",destinationBerth:"Destination approach and berth",limits:"Explicit operating limits and gates",abortDecision:"Go/delay/divert/abort triggers",alternatives:"Safe alternatives",manualVerification:"Independent manual verification"} as const)[key]}</Label><Textarea id={`safety-${key}`} value={plan.safety[key]} onChange={event=>setSafety(key,event.target.value)}/></div>)}</fieldset>
    ...
    <p className="rounded-md border border-amber-600 p-3 text-sm"><strong>Manual verification required:</strong> This plan and geometric review do not prove hazard clearance. Independently check the plotted track, depths, tidal heights and streams, charted dangers, notices, traffic and local rules using current official information.</p>
    {plan.points.map((point, index) => <Card key={point.id} className="break-inside-avoid">
      ...
      <CardContent className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {(["name","latitude","longitude"] as const).map(key => {const coordinateInvalid=(key==="latitude"||key==="longitude")&&!parseWaypointCoordinate(point.latitude,point.longitude);return <div key={key}><Label htmlFor={`${point.id}-${key}`}>{({ name:index === 0 ? "Departure waypoint" : index === plan.points.length - 1 ? "Destination waypoint" : "Arrival waypoint", latitude:"WGS84 latitude (DD°MM.mmm'N/S)", longitude:"WGS84 longitude (DDD°MM.mmm'E/W)" } as const)[key]}</Label><Input id={`${point.id}-${key}`} aria-invalid={coordinateInvalid||undefined} aria-describedby={coordinateInvalid?`${point.id}-coordinate-error`:undefined} value={point[key]} onChange={event => updatePoint(point.id, key, event.target.value)}/>{coordinateInvalid&&key==="longitude"&&<p id={`${point.id}-coordinate-error`} className="text-sm text-destructive">Enter in degrees and decimal minutes, with valid ranges and hemisphere.</p>}</div>})}
        {point.inboundLeg && <>
