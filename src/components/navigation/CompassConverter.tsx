      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-start mb-8">
          <div className="flex flex-col gap-2">
            ...
            <Input id="true" aria-label="True heading" aria-invalid={Boolean(result.trueField.error)} aria-describedby={result.trueField.error ? "true-error" : undefined} type="number" min="0" max="359" step="any" placeholder="000" value={trueHeading} onChange={(e) => setTrueHeading(e.target.value)} className="text-center font-bold text-lg" />
          ...
          <div className="flex flex-col gap-2 relative">
            ...
            <div className="flex gap-1 justify-center items-center"><span className="text-sm font-bold">{variationDir === "E" ? "-" : "+"}</span><Input id="variation" aria-label="Variation" aria-invalid={Boolean(result.variationField.error)} aria-describedby={result.variationField.error ? "variation-error" : undefined} type="number" min="0" max="90" step="any" placeholder="0" value={variation} onChange={(e) => setVariation(e.target.value)} className="w-16 h-8 text-center" /><Button type="button" aria-label={`Variation direction ${variationDir}`} variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => setVariationDir((prev) => prev === "E" ? "W" : "E")}>{variationDir}</Button></div>
          ...
          <div className="flex flex-col gap-2 text-center opacity-70"><Label>Magnetic (°M)</Label><output aria-label="Magnetic heading" className="h-10 flex items-center justify-center font-mono text-xl bg-muted rounded-md border border-input">{result.magneticHeading === undefined ? "--" : displayHeading(result.magneticHeading)}</output></div>
          <div className="flex flex-col gap-2 relative">
            <Label htmlFor="deviation" className="text-center text-xs text-muted-foreground">Deviation</Label>
            <div className="flex gap-1 justify-center items-center"><span className="text-sm font-bold">{deviationDir === "E" ? "-" : "+"}</span><Input id="deviation" aria-label="Deviation" aria-invalid={Boolean(result.deviationField.error)} aria-describedby={result.deviationField.error ? "deviation-error" : undefined} type="number" min="0" max="90" step="any" placeholder="0" value={deviation} onChange={(e) => setDeviation(e.target.value)} className="w-16 h-8 text-center" /><Button type="button" aria-label={`Deviation direction ${deviationDir}`} variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => setDeviationDir((prev) => prev === "E" ? "W" : "E")}>{deviationDir}</Button></div>
            {error("deviation-error", result.deviationField.error)}
            <MoveRight className="w-4 h-4 text-muted-foreground absolute -right-3 top-1/2 -translate-y-1/2 hidden md:block" />
          </div>
          <div className="flex flex-col gap-2"><Label className="text-primary font-bold">Compass (°C)</Label><output aria-label="Compass heading" className="h-10 flex items-center justify-center font-mono text-xl font-bold bg-primary/10 text-primary rounded-md border border-primary/30">{result.compassHeading === undefined ? "--" : displayHeading(result.compassHeading)}</output>{result.compassHeading !== undefined && <span className="text-xs text-center text-muted-foreground">Course to steer</span>}</div>
        </div>
