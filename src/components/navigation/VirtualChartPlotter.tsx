...
export default function VirtualChartPlotter({ onMastery }: VirtualChartPlotterProps) {
  ...
  const zoomPercent = Math.round(100 / view.scale);
  return <Card className="mt-8 min-w-0 w-full border-2 border-primary/20">
    <CardHeader className="px-3 sm:px-6"><CardTitle className="flex flex-col items-start gap-3 lg:flex-row lg:justify-between"><span className="flex items-center gap-2"><MapIcon aria-hidden className="h-5 w-5"/>Virtual Chart Plotter</span><span className="flex max-w-full flex-wrap gap-2">
      <Button type="button" aria-label="Zoom in" aria-describedby="plotter-scale-status" disabled={view.scale <= .5} onClick={() => setView((v) => clampView({ ...v, scale: Math.max(.5, v.scale - .25) }))}>+</Button><Button type="button" aria-label="Zoom out" aria-describedby="plotter-scale-status" disabled={view.scale >= 2.5} onClick={() => setView((v) => clampView({ ...v, scale: Math.min(2.5, v.scale + .25) }))}>−</Button>
      ...
      <p id="plotter-scale-status" role="status" aria-live="polite" className="text-xs font-normal text-muted-foreground">Current scale: {zoomPercent}%. Zoom range: 40% to 200%.</p>
    </CardHeader><CardContent className="min-w-0 px-3 sm:px-6">
      <div role="region" aria-label="Interactive practice chart" className="overflow-auto rounded-xl border bg-white">
        <svg ref={svgRef} role="img" aria-labelledby="chart-title chart-desc" tabIndex={0} viewBox={`${view.x} ${view.y} ${viewSize.width} ${viewSize.height}`} preserveAspectRatio="none" className={`block h-[min(60vh,480px)] min-h-[300px] w-full ${tool === "pan" ? "touch-pan-y" : "touch-none"}`}
      ...
      {active && <form className="mt-4 min-w-0 rounded border p-3" onSubmit={(e) => { e.preventDefault(); const parts = formValue.trim().split(/[ ,]+/).map(Number); if (challenge.kind === "plot" && parts.length === 2 && parts.every(Number.isFinite)) assess(chartModel.toPoint(parts[0], parts[1]), true); else if (parts.length === 1 && Number.isFinite(parts[0])) assess(parts[0], true); else setFeedback({ ok: false, text: "Enter one numeric measurement, or decimal latitude and longitude separated by a comma." }); }}><label htmlFor="plotter-answer" className="block font-medium">Equivalent nonvisual answer</label><p id="plotter-answer-help" className="text-sm text-muted-foreground">Enter distance in NM or bearing in degrees. For a plotted position, enter decimal latitude, longitude (west is negative).</p><div className="mt-2 flex min-w-0 flex-col gap-2 sm:flex-row"><input id="plotter-answer" aria-describedby="plotter-answer-help" className="min-h-11 min-w-0 flex-1 rounded border px-3" inputMode="decimal" disabled={challengeSolved} value={formValue} onChange={(e) => setFormValue(e.target.value)}/><Button type="submit" disabled={challengeSolved}>Check answer</Button></div></form>}
      ...
      <p className="mt-2 text-center text-xs text-muted-foreground">Local tangent-plane scale: 100 chart units = 1 NM. Latitude ticks are 0.5′; longitude spacing is corrected by cos 50°15′. With Pan selected, one-finger touch scrolls the page and the four Pan chart buttons move the chart. Plot, Distance and Bearing temporarily reserve touch gestures for accurate chart work; switch back to Pan to resume page scrolling. Mouse and pen users may also drag with Pan selected.</p>
    </CardContent></Card>;
}
