...
describe("passage calculations", () => {
  ...
  it("treats routed detours as advisory and flags opposite-direction courses",()=>{const route:PlanWaypoint[]=[{id:"a",name:"A",latitude:"50°00.0'N",longitude:"001°00.0'W",inboundLeg:null},{id:"b",name:"B",latitude:"50°03.0'N",longitude:"001°00.0'W",inboundLeg:{course:180,distanceNm:20,notes:"",tidalGate:"",weatherWindow:""}}];expect(routeGeometryIssues(route)).toEqual(expect.arrayContaining([expect.stringContaining("intended detour"),expect.stringContaining("differs from endpoint bearing")]))});
});
