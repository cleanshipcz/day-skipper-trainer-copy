...
import { PASSAGE_PLAN_CACHE_VERSION, calculatePassagePlanSummary, decodePassagePlanCache, decodePassagePlanRecord, insertWaypoint, makePassagePlanRecord, normalizeWaypointOrder, parsePassagePlanCache, passagePlanCacheKey, reconcilePassagePlanRecords, removeWaypoint, reorderWaypoint, validatePassagePlan, type PassagePlan } from "./passagePlan";
...
describe("passage plan validation and cache",()=>{
 ...
 it("returns a structured failure rather than throwing for a recoverable one-waypoint plan with invalid departure",()=>{const plan={...valid,departure:"invalid",points:[valid.points[0]]};expect(()=>calculatePassagePlanSummary(plan)).not.toThrow();expect(calculatePassagePlanSummary(plan)).toMatchObject({ok:false,issues:expect.arrayContaining([expect.stringContaining("departure time")])})});
 it.each([{points:[]},{points:[valid.points[0]]}])("returns structured unavailable totals for an incomplete route",({points})=>{const result=calculatePassagePlanSummary({...valid,points});expect(result).toMatchObject({ok:false,issues:expect.arrayContaining([expect.stringContaining("departure and at least one destination")])})});
 it("enforces the derived fuel ceiling with an omitted zero reserve",()=>{const plan={...valid,speed:2,fuelRate:500,reservePercent:undefined,points:[valid.points[0],{...valid.points[1],inboundLeg:{...valid.points[1].inboundLeg!,distanceNm:2000}}]};expect(validatePassagePlan(plan)).toContain("Derived total fuel must be finite, positive and no more than 100,000 litres.");expect(calculatePassagePlanSummary(plan)).toMatchObject({ok:false,issues:expect.arrayContaining([expect.stringContaining("100,000 litres")])})});
 ...
 it("reconciles revisioned records deterministically and detects equal-head conflicts",()=>{const older=makePassagePlanRecord(valid,"user-a",null,"2026-01-01T00:00:00Z"),newer=makePassagePlanRecord({...valid,name:"newer"},"user-a",older,"2026-01-02T00:00:00Z");expect(reconcilePassagePlanRecords(older,newer)).toMatchObject({status:"remote",record:{revision:2}});const divergent={...newer,plan:{...newer.plan,name:"other"}};expect(reconcilePassagePlanRecords(newer,divergent)?.status).toBe("conflict")});
 it("invalidates completion on a material revision and rejects cross-owner records",()=>{const completed={...makePassagePlanRecord(valid,"user-a"),completedRevision:1,completionStatus:"confirmed" as const};const edited=makePassagePlanRecord({...completed.plan,name:"edited"},"user-a",completed);expect(edited.completedRevision).toBeNull();expect(decodePassagePlanRecord(JSON.stringify(completed),"user-b").ok).toBe(false)});
 it("never downgrades confirmed completion when equal plan revisions reconcile",()=>{const base=makePassagePlanRecord(valid,"user-a");const queued={...base,completedRevision:base.revision,completionStatus:"queued" as const},confirmed={...base,completedRevision:base.revision,completionStatus:"confirmed" as const,updatedAt:"2020-01-01T00:00:00Z"};expect(reconcilePassagePlanRecords(queued,confirmed)).toMatchObject({status:"same",record:{completedRevision:1,completionStatus:"confirmed"}})});
 it("distinguishes descendants from independent device lineages",()=>{const base=makePassagePlanRecord(valid,"user-a",null,"2026-01-01T00:00:00Z");const deviceA=makePassagePlanRecord({...valid,name:"A"},"user-a",base,"2026-01-02T00:00:00Z");const deviceB=makePassagePlanRecord({...valid,name:"B"},"user-a",base,"2026-01-03T00:00:00Z");const descendant=makePassagePlanRecord({...valid,name:"A2"},"user-a",deviceA,"2026-01-04T00:00:00Z");expect(reconcilePassagePlanRecords(deviceA,deviceB)?.status).toBe("conflict");expect(reconcilePassagePlanRecords(deviceA,descendant)).toMatchObject({status:"remote",record:{plan:{name:"A2"}}})});
});
