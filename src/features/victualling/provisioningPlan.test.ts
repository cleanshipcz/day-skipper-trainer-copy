...
describe("calculateProvisioningPlan", () => {
  it("keeps units and decimal arithmetic explicit", () => {
    expect(calculateProvisioningPlan(passage)).toEqual({
      ...
      totalWaterLitres: 92, usableWaterCapacityLitres: 95, waterShortfallLitres: 0,
  ...
  it("warns when realistic usable capacity is insufficient", () => {
    expect(calculateProvisioningPlan({ ...passage, tankCapacityLitres: 80, unusableTankLitres: 10 })).toMatchObject({ usableWaterCapacityLitres: 70, waterShortfallLitres: 22, fuelShortfall: 0.4 });
  });
