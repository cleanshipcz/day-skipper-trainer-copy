...
describe("review question registry", () => {
  test("should resolve every shared quiz question without duplicating definitions", async () => {
  ...
  test("should reject duplicate stable question ids across topics", async () => {
    const quizRegistry = await loadAllQuizTopics();
  ...
  test("should filter due records, ignore unknown ids, and order oldest first", async () => {
    const quizRegistry = await loadAllQuizTopics();
