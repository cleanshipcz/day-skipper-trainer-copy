...
export const normalizeRigCatalogue = (value: readonly RigCheck[]) => {
  ...
  return value.filter((item) => {
    if (!item || !item.id.trim() || !item.area.trim() || !item.item.trim() || !item.lookFor.trim() || !item.acceptableEvidence.trim() || !item.limitations.trim() || !item.boundary.trim() || seen.has(item.id)) return false;
