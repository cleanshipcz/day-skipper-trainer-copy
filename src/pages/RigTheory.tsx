...
const RigTheory = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-ocean-light/10 to-background">
    ...
    <main className="container mx-auto max-w-5xl px-3 py-6 sm:px-4 sm:py-8">
      ...
      <Card className="mb-6 border-2 border-destructive/60"><CardContent className="flex gap-3 pt-6"><AlertCircle aria-hidden="true" className="mt-0.5 size-6 shrink-0 text-destructive"/><div><h2 className="font-bold">Control the hazards before looking</h2><p className="mt-2 text-sm text-muted-foreground">Keep the mast and all rigging well clear of overhead electrical conductors—contact is not required for high voltage to arc. Depower sails; control the boom; keep clear of bights, snap-back paths, winches and loaded lines; and protect against sharp wire. Do not go aloft for this review. Aloft work requires a competent plan, independent fall protection, communication, controlled tools and suitable conditions.</p></div></CardContent></Card>
      <Card className="mb-6"><CardHeader><CardTitle>Learning objectives</CardTitle></CardHeader><CardContent><ul className="list-disc space-y-2 pl-5 text-sm text-muted-foreground"><li>Orient standing rigging, spars, terminals and attachments, and running-rigging paths against the fitted vessel plan.</li><li>Locate normal and abnormal evidence without entering stored-energy, fall, boom-swing or electrical hazards.</li><li>Distinguish acceptable recorded evidence from uncertainty, understand visual limits, and take a bounded defect action.</li><li>Conduct a coherent deck-level pre-passage walk-round and recognise incident-driven inspection triggers.</li></ul></CardContent></Card>
      ...
      <Card className="mb-6"><CardHeader><CardTitle>Applied evidence walk-round</CardTitle></CardHeader><CardContent><p className="mb-4 text-sm text-muted-foreground">Work one observation at a time: locate the relevant component, evaluate what the evidence can support, and apply the safe boundary. This is deliberately not a check-all exercise.</p><RigEvidencePractice onComplete={() => setPracticeComplete(true)} /></CardContent></Card>
      ...
      <Card className={`mt-6 border-2 ${qualified ? "border-success" : "border-accent"}`}><CardContent className="flex flex-col gap-4 pt-6 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-bold">{qualified ? "Learning review complete" : "Readiness not established"}</h2><p className="text-muted-foreground">{qualified ? "All items have satisfactory evidence recorded and the applied practice is complete. This still is not a certificate of vessel condition." : "Finish the applied practice and every item, then resolve defects or uncertainty before any qualified readiness statement."}</p></div><Button onClick={() => navigate("/quiz/rig")}>Practise Rig Quiz</Button></CardContent></Card>
    </main>
  </div>;
};
