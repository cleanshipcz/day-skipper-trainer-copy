...
const RigTheory = () => {
  ...
  return <div className="min-h-screen min-w-0 overflow-x-hidden bg-gradient-to-br from-background via-ocean-light/10 to-background motion-reduce:scroll-auto">
    ...
    <main className="container mx-auto max-w-5xl px-3 py-6 sm:px-4 sm:py-8">
      ...
      <div className="mb-4 text-sm" role={status === "failed" ? "alert" : "status"}>{status === "loading" && "Loading rig review…"}{status === "saving" && "Saving rig review…"}{status === "saved" && "Rig review saved."}{status === "anonymous" && "Rig review saved for this browser session."}{status === "queued" && "Rig review queued offline; server save is not yet confirmed."}{status === "failed" && <span>{pending ? "Your latest outcome was not saved." : failure === "malformed-remote" ? "Saved rig review uses an incompatible catalogue; it was not changed." : failure === "malformed-anonymous" ? "This browser's rig review is malformed; it was not deleted." : failure === "catalogue" ? "Rig review is unavailable because its catalogue is invalid." : "Saved rig review could not be loaded; editing is paused."} {pending && <Button className="min-h-11 min-w-11" size="sm" variant="outline" onClick={() => void persist(pending)}>Retry save</Button>}{!pending && (failure === "transient" || failure === "malformed-anonymous") && <Button className="min-h-11 min-w-11" size="sm" variant="outline" onClick={() => setLoadAttempt((n) => n + 1)}>Retry load</Button>}{failure === "malformed-anonymous" && <Button className="min-h-11 min-w-11" size="sm" variant="outline" onClick={clearMalformedAnonymous}>Clear local review</Button>}{failure === "malformed-remote" && <Button className="min-h-11 min-w-11" size="sm" variant="outline" onClick={resetMalformedRemote}>Reset saved review</Button>}</span>}</div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><p id="rig-review-progress" role="status" aria-live="polite" aria-atomic="true">Rig review progress: {reviewed} of {catalogue.length} items reviewed; {blockers} unresolved.</p><Button className="min-h-11 min-w-11" variant="outline" onClick={reset} disabled={reviewed === 0 || status === "saving"}>Reset review</Button></div>
      ...
      <div role="status" aria-live="polite" aria-atomic="true" className="sr-only">{qualified ? "Learning review complete. Continue to the Rig quiz." : practiceComplete ? "Evidence walk-round practice complete. Continue reviewing every rig item." : ""}</div>
      <Card id="rig-handoff" className={`mt-6 border-2 forced-colors:border-[CanvasText] ${qualified ? "border-success" : "border-accent"}`}><CardContent className="flex min-w-0 flex-col gap-4 pt-6 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><h2 className="break-words text-xl font-bold">{qualified ? "Learning review complete" : "Readiness not established"}</h2><p className="break-words text-muted-foreground">{qualified ? "All items have satisfactory evidence recorded and the applied practice is complete. Continue with the Rig quiz using the button below. This still is not a certificate of vessel condition." : "Finish the applied practice and every item, then resolve defects or uncertainty before any qualified readiness statement."}</p></div><Button className="min-h-11 min-w-11 shrink-0" onClick={() => navigate("/quiz/rig")}>Practise Rig Quiz</Button></CardContent></Card>
    </main>
  </div>;
};
