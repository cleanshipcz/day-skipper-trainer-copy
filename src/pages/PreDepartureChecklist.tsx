...
export default function PreDepartureChecklist() {
  ...
  const focusSummaryAfterSave = useRef(false);
  ...
  useEffect(() => {
    if (completionConfirmed && focusSummaryAfterSave.current) {
      focusSummaryAfterSave.current = false;
  ...
  return (
    <main className="container mx-auto max-w-4xl space-y-6 p-4 py-8 [overflow-wrap:anywhere]">
          ...
          {preDepartureChecklist.filter((item) => item.phase === phase).map((item) => {
            ...
            return <fieldset key={item.id} className="min-w-0 space-y-3 rounded border-2 p-3 sm:p-4 forced-colors:border-[CanvasText]" data-phase={phase} aria-describedby={describedBy}>
              ...
              <div className="flex flex-wrap gap-2" aria-label={`${item.label} status choices`} aria-describedby={describedBy}>
                {statuses.filter((status) => canSelectStatus(item, status)).map((status) => <Button key={status} type="button" size="sm" className="min-h-11 min-w-11 max-w-full whitespace-normal [overflow-wrap:anywhere] forced-colors:border forced-colors:border-[ButtonText] motion-reduce:transition-none" variant={entry.status === status ? "default" : "outline"} aria-pressed={entry.status === status} disabled={!catalogue.valid || loadState !== "ready" || (dependencyBlocked && status !== "not_checked")} onClick={() => setStatus(item.id, status)}>{readinessStatusLabels[status]}</Button>)}
              </div>
      ...
      <Button className="min-h-11 w-full whitespace-normal sm:w-auto motion-reduce:transition-none" aria-describedby={!contextComplete ? "readiness-context-requirement" : undefined} disabled={!completionReady || loadState !== "ready" || saveState === "saving"} onClick={() => { focusSummaryAfterSave.current = true; const currentRevision = revision.current; void persistRecord(true, currentRevision); }}>Record training checklist completion</Button>
    </main>
  );
}
