              ...
              <Card className="min-w-0 overflow-hidden">
                <CardContent className="min-w-0 p-4 md:p-6">
                  <div className="relative min-w-0">
                    ...
                    <p id="sail-controls-diagram-help" data-schematic-help className="mb-3 text-center text-sm text-muted-foreground">
                      Follow a coloured route from sail to deck. Match its numbered marker to the diagram key, then select either one to isolate it.
                    </p>
                    <div
                      data-schematic-scroll
                      className="mx-auto w-full min-w-0 max-w-4xl overflow-x-auto overscroll-x-contain rounded-md"
                    >
                      <SailControlsDiagram
                    ...
                    <div className="mx-auto mt-4 w-full min-w-0 max-w-4xl" aria-label="Diagram number key">
                      <p className="mb-2 text-sm font-medium">Diagram key</p>
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
                        {sailControls.map((control, index) => (
                          <button
                            key={control.id}
                            type="button"
                            className="flex min-h-11 items-center gap-2 rounded-md border bg-background px-3 py-2 text-left text-sm hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                            aria-label={`Show ${control.name} details from diagram key`}
                            ...
                            onFocus={() => setHoveredPart(control)}
                            onBlur={() => setHoveredPart(null)}
                          >
                            <span
                              className="flex h-7 w-7 flex-none items-center justify-center rounded-full border-[3px] bg-white text-xs font-extrabold text-slate-900"
                              style={{ borderColor: control.color }}
                              aria-hidden="true"
                            >
                              {index + 1}
                            </span>
                            <span className="leading-tight">{control.name}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    {!selectedPart && (
                      <div data-schematic-hint className="mx-auto mt-4 w-fit max-w-full">
                ...
                >
                  <CardContent className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <span className="h-4 w-4 flex-shrink-0 rounded-full border-2 border-white shadow-sm" style={{ backgroundColor: control.color }} aria-hidden="true" />
