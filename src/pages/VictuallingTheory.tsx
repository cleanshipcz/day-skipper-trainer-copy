  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-ocean-light/10 to-background">
    ...
    <main className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      <div className="mb-4 text-sm" role={status === "conflict" || status === "failed" ? "alert" : undefined} aria-atomic={status === "conflict" || status === "failed" ? "true" : undefined}>
        ...
        {status === "conflict" && <span className="inline-flex flex-wrap items-center gap-3">This checklist changed elsewhere and your latest change was not saved. Editing is paused. Reload checklist is available to get the latest version.<Button size="sm" variant="outline" onClick={() => setLoadRevision((value) => value + 1)}>Reload checklist</Button></span>}
        {status === "failed" && <span className="inline-flex flex-wrap items-center gap-3">{pendingIds ? "Your latest checklist change was not saved. Editing is paused; Retry save is available." : "Saved checklist could not be loaded. Editing is paused to protect your existing progress; Retry load is available."}<Button size="sm" variant="outline" onClick={() => pendingIds ? void persist(pendingIds) : setLoadRevision((value) => value + 1)}>{pendingIds ? "Retry save" : "Retry load"}</Button></span>}
      </div>
      ...
      {total > 0 && count === total && <Card className="border-2 border-accent bg-accent/5 forced-colors:border-[CanvasText]" role="region" aria-label="Victualling checklist complete"><CardContent className="pt-6"><div className="flex flex-col items-stretch gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><h3 className="text-xl font-bold mb-2 break-words">Victualling checklist complete</h3><p className="text-muted-foreground break-words">This does not confirm the vessel is ready for sea. Complete the separate safety and pre-departure checks; the quiz is this topic's learning completion gate.</p></div><Button size="lg" className="w-full shrink-0 sm:w-auto" onClick={() => navigate("/quiz/victualling")}>Take Quiz</Button></div></CardContent></Card>}
    </main>
  </div>;
};
