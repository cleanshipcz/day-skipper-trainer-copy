-- Retire safety/personal inventory IDs now owned by dedicated learning areas.
-- Preserve each row's compare-and-swap revision while filtering its selection.
update public.user_progress p
set answers_history = jsonb_set(
      p.answers_history,
      '{checkedItemIds}',
      coalesce((
        select jsonb_agg(item_id order by item_id)
        from (
          select distinct value #>> '{}' as item_id
          from jsonb_array_elements(p.answers_history->'checkedItemIds') value
          where jsonb_typeof(value) = 'string'
            and value #>> '{}' = any (array['f1','f2','f3','f4','f5','g1','g2','g3','g4','g5']::text[])
        ) retained
      ), '[]'::jsonb),
      false
    ),
    completed = false,
    score = 0
where p.topic_id = 'victualling-checklist'
  and p.answers_history->>'version' = '1'
  and jsonb_typeof(p.answers_history->'checkedItemIds') = 'array';
