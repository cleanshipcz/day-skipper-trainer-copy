...
try {
  ...
  const audit = await evaluate(`(()=>{const main=document.querySelector('main');const buttons=[...document.querySelectorAll('fieldset button')].map((node)=>node.getBoundingClientRect().toJSON());const progress=document.querySelector('[role=progressbar]');const first=document.querySelector('fieldset');const overflow=[...document.querySelectorAll('main *')].filter(node=>node.getBoundingClientRect().right>321).slice(0,5).map(node=>({tag:node.tagName,text:node.textContent.slice(0,60),right:node.getBoundingClientRect().right,class:node.className}));return {viewport:document.documentElement.clientWidth,width:document.documentElement.scrollWidth,mainRight:main.getBoundingClientRect().right,overflow,buttons,progressName:progress.getAttribute('aria-label'),progressDescription:progress.getAttribute('aria-describedby'),itemName:first.querySelector('legend').textContent,itemDescription:first.getAttribute('aria-describedby'),reduced:matchMedia('(prefers-reduced-motion: reduce)').matches,forced:matchMedia('(forced-colors: active)').matches}})()`);
  ...
  if (!audit.progressName || !audit.progressDescription || !audit.itemName || !audit.itemDescription.includes("rationale") || !audit.reduced || !audit.forced) throw new Error(`semantic/media audit failed ${JSON.stringify(audit)}`);
  ...
  console.log("Readiness accessibility browser checks passed: keyboard activation without focus theft, concise live status, names/descriptions, 44px targets, 320px at 200% reflow, forced colors and reduced motion.");
} finally {
