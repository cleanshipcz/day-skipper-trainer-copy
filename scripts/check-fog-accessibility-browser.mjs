...
try {
  ...
  const semantics = await evaluate(`(() => { const practice=document.querySelector('[aria-labelledby="fog-practice-heading"]'); const figure=practice.querySelector('figure'); const svg=figure.querySelector('svg[role=img]'); const title=svg.querySelector('title'); const caption=figure.querySelector('figcaption'); const buttons=[...practice.querySelectorAll('button')].filter((node)=>getComputedStyle(node).display!=='none').map((node)=>node.getBoundingClientRect().toJSON()); const choices=[...practice.querySelectorAll('label')].map((node)=>node.getBoundingClientRect().toJSON()); return {viewport:document.documentElement.clientWidth,pageWidth:document.documentElement.scrollWidth,practice:practice.getBoundingClientRect().toJSON(),title:title?.textContent,caption:caption?.textContent,labelledby:svg.getAttribute('aria-labelledby'),relations:[title?.id,caption?.id],captionVisible:getComputedStyle(caption).display!=='none',animation:getComputedStyle(practice.querySelector('article')).animationName,darkApplied:document.documentElement.classList.contains('dark'),darkBackground:getComputedStyle(figure).backgroundColor,reduced:matchMedia('(prefers-reduced-motion: reduce)').matches,buttons,choices}; })()`);
  ...
  if (semantics.title !== "Operational situation diagram" || !semantics.captionVisible || !semantics.caption?.includes("Diagram meaning:") || semantics.labelledby !== semantics.relations.join(" ") || !semantics.relations.every(Boolean) || !semantics.reduced || semantics.animation !== "none" || !semantics.darkApplied) throw new Error(`Fog media/semantic equivalence failed: ${JSON.stringify(semantics)}`);
  await send("Emulation.setEmulatedMedia", { features: [{ name: "prefers-reduced-motion", value: "reduce" }, { name: "prefers-color-scheme", value: "light" }] }); await evaluate("document.documentElement.classList.remove('dark')"); await delay(100);
  const lightStyle = await evaluate("(() => { const figure=document.querySelector('[aria-labelledby=\"fog-practice-heading\"] figure'); return {lightApplied:!document.documentElement.classList.contains('dark'),background:getComputedStyle(figure).backgroundColor,animation:getComputedStyle(figure.closest('article')).animationName}; })()");
  if (!lightStyle.lightApplied || lightStyle.background === semantics.darkBackground || lightStyle.animation !== "none") throw new Error(`Fog rendered theme/reduced-motion behavior did not change safely: ${JSON.stringify({ dark: semantics.darkBackground, light: lightStyle })}`);
  ...
  await focusSelector('[aria-label="Scenario progress"] button:nth-child(2)'); await pressKey("Enter", "Enter", 13);
  await waitFor(() => evaluate("document.activeElement?.tagName === 'H3' && document.activeElement.textContent.includes('Respond to sudden visibility loss')"), "scenario heading focus handoff");
  await focusSelector('[aria-label="Scenario progress"] button:nth-child(1)'); await pressKey("Enter", "Enter", 13);
  await waitFor(() => evaluate("document.activeElement?.tagName === 'H3' && document.activeElement.textContent.includes('Recognise a developing fog risk')"), "restored heading focus handoff");
  const restored = await evaluate(`(() => { const button=[...document.querySelectorAll('button')].find((node)=>node.textContent.includes('Scenario 1 — complete')); const fieldset=document.querySelector('[aria-labelledby="fog-practice-heading"] fieldset'); return {current:button.getAttribute('aria-current'),disabled:fieldset.disabled,status:document.querySelector('[aria-labelledby="fog-practice-heading"] [role=status]')?.textContent,focused:document.activeElement?.tagName}; })()`);
